...
def send_message(route):
    # compute travel+delay time
    # format to string and strip trailing zeros
    value = str((sum(route) / 300000) + (len(route) - 1) * 0.5).rstrip('0')
    # if more than 4 decimal places, format to 4 decimal places
    if len(value[value.find("."):]) > 4:
